...
from processing import TqdmToLogger
...
class Test:
    ...
    def _check(
        self,
        test: pd.DataFrame,
        task1_prediction: pd.DataFrame,
        task2_prediction: pd.DataFrame,
    ):
        ...
        if set(range(test.shape[0])) != set(task1_prediction["index"]):
            error = "В ответах к задаче 1 должны быть `index` должен полностью пересекаться с тестом"
            raise ValueError(error)
    ...
    def test(self) -> pd.DataFrame:
        path = os.path.join(
            self.test_data_dir, self.val_csv if self.debug else self.test_csv
        )
        if not os.path.exists(path):
            self.logger.info(f"Файл {path} не найден")
    ...
    def run(self):
        ...
        task1_path = os.path.join(
            self.test_data_dir, f"{self.user}_{self.task1_prediction}"
        )
        task2_path = os.path.join(
            self.test_data_dir, f"{self.user}_{self.task2_prediction}"
        )
    ...
    def process(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        task1_prediction["prediction"] = task1(test["description"], tqdm_out)
